...
const STEPS = [
  { id: '1', title: 'Ciblage avancé', duration: '10 min', description: 'Ciblage main/objet avec critères stables et distractions légères.' },
  { id: '2', title: 'Impulsions maîtrisées', duration: '8 min', description: 'Attendre le signal avant action très motivante.' },
  { id: '3', title: 'Travail à distance', duration: '6 min', description: 'Signaux à 3–5 m, retour rapide et précis.' },
];

export default function ParcoursExpertList() {
  const navigation = useNavigation();
  const { doneMap, toggle, resetAll } = useTrackProgress('expert');

  const goDetail = (step, done) => {
    navigation.navigate('StepDetail', { track: 'expert', step, done: !!done });
  };
